		...
		for (x, y, window) in sliding_window(resized, stepSize=stepSize, windowSize=(winW, winH)):
			...
			if (predicted == torch.tensor([1])):
				...
				if face_prob > confidence_required:
					...
					# detect x,y positions in the original size of the image and save them
...
# Show all detected faces with a red rectangle in the final image aftter applying nms
